...
// +ANTON -- when not 0, flips that produce score >= iMaxBreakValue are blocked
UBIGINT iMaxBreakValue = 0;
// -ANTON
...
void PickWalkSatSKCW() {
  ...
  // +ANTON: get out if the flip is bad
  if (iMaxBreakValue && (iSumFalseWeight < iMaxBreakValue) 
      && ((UBIGINT)iBestScoreWeight >= iMaxBreakValue)) {
    iFlipCandidate = 0;
    return;
  }
  // -ANTON
